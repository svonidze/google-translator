...
void main() {
  ...
  test('Pronounce from Russian', () async {
    final url = await translator.getPronunciationUrl('хорошо', from: 'ru');
    print(url);
  });
  ...
  test('Examples from English and from AUTO', () async {
    ...
    expect(unorderedCollectionEquality.equals(englishResponse.examples, expected), true);

    final autoResponse = await translator.getExamples('discrimination');
    expect(unorderedCollectionEquality.equals(autoResponse.examples, expected), true);
  });
